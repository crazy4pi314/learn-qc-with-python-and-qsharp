// operations.qs: Sample code for Hamiltonian simulation (Chapter 9).
//
...
namespace HamiltonianSimulation { 
    ...
    /// ```
    /// For the example here, the Hamiltonian is:
    /// H ≔ a II + b₀ ZI + b₁ IZ + b₂ ZZ + b₃ YY + b₄ XX
    ///

    // tag::h2_form[]
    function H2Terms(idxHamiltonian : Int) : (Pauli[], Int[]) {               // <1>
        return [
            ([PauliZ], [0]),                                                  // <2>
            ([PauliZ], [1]), 
            ([PauliZ, PauliZ], [0, 1]),                                       // <3>
    ...
    operation PrepareInitalState(q : Qubit[]) : Unit {                        // <4>
        X(q[0]);
    }
    // end::h2_form[]
    ...
    operation EstimateH2Energy(idxBondLength : Int) : Double {                // <1>
        ...
        let trotterStep = ApplyHamiltonian(idxBondLength, trotterStepSize, _);// <4>
